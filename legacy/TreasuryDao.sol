...
contract TreasuryDao is ITreasuryDao, Ownable {
    ...
    function _delegate (address _community, address _currency, uint256 _amount) internal {
        (,, address debtTokenAddress) = aaveProtocolDataProvider.getReserveTokensAddresses(_currency);
        ICreditDelegationToken(debtTokenAddress).approveDelegation(_community, _amount);
