...
const expectedDelegations = ["500", "0"];
...
describe("Deposit and borrow happy flow", function() {
    describe("Deployment", function () {
        ...
        it("Should deploy contract factories", async function() {
            const CommunityTreasuryFactory = await ethers.getContractFactory("CommunityTreasuryFactoryNoAave");
        ...
        it("Should deploy Q2T contract", async function() {
            const Q2T = await ethers.getContractFactory("Q2TAaveless");
    ...
    describe("Basic flow", function() {
        ...
        it("Should deposit DAI to Q2T for template 1", async function() {
            ...
            const Q2T = await ethers.getContractFactory("Q2TAaveless", signer);
            ...
            expect(await daiToken.balanceOf(q2tImp.address)).to.equal("1000".concat(e18));
        });
        ...
        it("Should deposit more DAI to Q2T for template 1", async function() {
            await hre.network.provider.request({
                method: "hardhat_impersonateAccount",
                params: [process.env.IMPERSONATE]
            });
            ...
            expect(await daiToken.balanceOf(q2tImp.address)).to.equal("1600".concat(e18));
        });
        it("Should NOT have issued  newtreasury NFT for template 1 and used existing one", async function() {
            ...
            expect(await templatesTreasuries.balanceOf(q2t.address, 2)).to.equal("0");
            expect(await templatesTreasuries.balanceOf(q2t.address, 3)).to.equal("0");
            expect(await templatesTreasuries.getCurrentFund(1)).to.equal("1000".concat(e18)); 
            expect(await templatesTreasuries.getCurrentFund(2)).to.equal("0");
            expect(await templatesTreasuries.getCurrentFund(3)).to.equal("0");
        ...
        it("Should deposit DAI to Q2T for template 3", async function() {
            await hre.network.provider.request({
                method: "hardhat_impersonateAccount",
                params: [process.env.IMPERSONATE]
            });
            ...
            expect(await daiToken.balanceOf(q2tImp.address)).to.equal("1700".concat(e18));
        });
        it("Should have issued treasury NFT for template 3 and assigned funds to it", async function() {
            ...
            expect(await templatesTreasuries.getCurrentFund(1)).to.equal("1000".concat(e18)); 
            expect(await templatesTreasuries.getCurrentFund(2)).to.equal("0");
            expect(await templatesTreasuries.getCurrentFund(3)).to.equal("70".concat(e18));
        ...
        it("Should delegate credit to communities treasuries after therhsold is reached", async function() {
            ...
            for (let i = 0; i < milestonesNum; i++) {
                ...
                expect(await q2t.delegatedCredit(communityTreasury, dai)).to.equal(expectedDelegations[i].concat(e18));
            }
        });
        ...
        it("Should have burnt treasury NFT for template 1 (leaving others alone)", async function() {
            ...
            expect(await templatesTreasuries.balanceOf(q2t.address, 5)).to.equal("0");
            expect(await templatesTreasuries.balanceOf(q2t.address, 6)).to.equal("0");
            expect(await templatesTreasuries.getCurrentReserve(1)).to.equal("500".concat(e18));
            expect(await templatesTreasuries.getCurrentReserve(2)).to.equal("0");
            expect(await templatesTreasuries.getCurrentReserve(3)).to.equal("0");
        ...
        it("Should borrow delegated credit and allocate it to project", async function() {
            ...
            const communityTreasury = await ethers.getContractAt("CommunityTreasuryNoAave", communityTreasuryAddress);
            const daiToken = await ethers.getContractAt("IERC20", dai);

            await communityTreasury.allocateDelegated();

            expect(await daiToken.balanceOf(communityTreasury.address)).to.equal("500".concat(e18));
            expect(await communityTreasury.projectAllocation(0)).to.equal("500".concat(e18));
        ...
        it("Should not allow receiving more than allocation", async function() {
            ...
            expect(
                communityTreasury.redeemAllocation("50".concat(e18).concat("1"), projectIds[0])
            ).to.be.revertedWith("< allocation");
        });
        it("Should receive allocated credit", async function() {
            ...
            await communityTreasury.redeemAllocation("500".concat(e18), projectIds[0]);

            const daiToken = await ethers.getContractAt("IERC20", dai);

            expect(await daiToken.balanceOf(communityMocks[0])).to.equal("500".concat(e18));
            expect(await communityTreasury.projectAllocation(0)).to.equal("0");
        });
        it("Should not be able to receive allocation again", async function() {
            ...
            expect(
                communityTreasury.redeemAllocation("1", projectIds[0])
            ).to.be.revertedWith("< allocation");
        });
    });
